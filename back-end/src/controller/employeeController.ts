...
class EmployeeController {
  setupEmployeeAccount = async (req: Request, res: Response): Promise<void> => {
    ...
    if (employee.email) {
      const employeeAccRes =
        await employeeService.setupEmployeeAccount(employee);
      if (employeeAccRes?.success) res.status(200).json(employeeAccRes);
      else res.status(400).json(employeeAccRes);
    } else {
      res
        .status(400)
        .json({ success: false, message: "Please provide employee's email" });
    }
  };
  ...
  async getAllEmployeesByCriteria(req: Request, res: Response): Promise<void> {
    try {
      const employees = await employeeService.getAllEmployeesByCriteria(
        req.body,
        true,
      );
      if (employees) {
  ...
  async createEmployee(req: Request, res: Response): Promise<void> {
    ...
    try {
      const result = await employeeService.createItem({
        name,
        email,
        department,
        role,
  ...
  async deleteEmployee(req: Request, res: Response): Promise<void> {
    ...
    try {
      const deleteEmployeeResult = await employeeService.deleteItem(
        employee.id,
      );
      const deleteEmployeeAccountResult =
        await userService.deleteUserByCriteria({ employeeId: employee.id });
      if (deleteEmployeeResult.success && deleteEmployeeAccountResult.success) {
        ...
        res
          .status(500)
          .json({ success: false, message: "Can not delete employee" });
      }
    } catch (error) {
    ...
    try {
      if (employee?.id) {
        ...
        if (result) {
          res
            .status(200)
            .json({ success: result, message: "Employee edited successfully" });
        } else {
          res
            .status(500)
            .json({ success: false, message: "Can not edit employee" });
        }
      }
    } catch (error) {
      console.error("Error deleting employee:", error);
