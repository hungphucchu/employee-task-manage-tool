...
class UserController {
  ...
  validateAccessCode = async (req: Request, res: Response): Promise<void> => {
    ...
    if (email && accessCode) {
      const validateAccessCodeRes = await userService.validateUser({
        email,
        accessCode,
      });
      if (validateAccessCodeRes.success)
        res.status(201).json(validateAccessCodeRes);
  ...
  validateUsernameAndPassword = async (
    ...
    if (username && password) {
      const validateSuccessRes = await userService.validateUser({
        username,
        password,
      });
  ...
  validateToken = async (req: CustomRequest, res: Response): Promise<void> => {
    if (req.user) {
      ...
      const validateTokenRes = await userService.getUserByCriteria(
        { username },
        true,
      );
    ...
    try {
      if (user?.id) {
        ...
        if (result) {
          res
            .status(200)
            .json({ success: result, message: "User edited successfully" });
        } else {
          res
            .status(500)
            .json({ success: false, message: "Can not edit user" });
        }
      }
    } catch (error) {
    ...
    try {
      if (getUserRes?.user?.id) {
        ...
        if (updateUser?.password) {
          updateUser = {
            ...updateUser,
            password: await utils.hashPassword(updateUser?.password),
          };
        }
        const result = await userService.updateItem(
          getUserRes?.user?.id,
          updateUser,
        );
        if (result) {
