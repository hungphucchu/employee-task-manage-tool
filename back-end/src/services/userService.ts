...
class UserService extends BaseService<User> {
  ...
  public deleteUserByCriteria = async (
    user: User,
    ...
    try {
      ...
      if (users && users.length > 0)
        getUserByCriteriaRes = {
          success: true,
          user: users[0],
          message: "Complete getting the user!",
        };
  ...
  public validateUserAlreadyExists = async (user: User): Promise<any> => {
    let validateUserAlreadyExistsRes = {
      success: true,
    ...
    try {
      ...
      if (userExists.length > 0)
        validateUserAlreadyExistsRes = {
          success: false,
          message: "Other user already sign up with this email",
        };
  ...
  public createNewUser = async (user: User): Promise<any> => {
    try {
      ...
      if (user?.email) {
        const userHasThisEmailRes = await userService.validateUserAlreadyExists(
          { email: user?.email },
        );
  ...
  public validateUser = async (user: User): Promise<any> => {
    ...
    try {
      if (user.username && user.password) {
        const users = await userRepo.getItemsByCriteria(
          { username: user.username },
          true,
        );
        if (users.length === 0) return false;
        if (user.password && users[0]?.password)
          valid = await utils.comparePassword(
            user?.password,
            users[0]?.password,
          );
        ...
        const ownerAccount = await userRepo.getItemsByCriteria(
          { accessCode: user.accessCode, email: user.email },
          true,
        );
        if (ownerAccount && ownerAccount.length > 0) {
          const newUserRes = await userRepo.updateItemWithCriteria(user, {
            username: userName,
          ...
          userRes = {
            ...user,
            ownerId: ownerAccount[0].id,
            id: ownerAccount[0].id,
          };
        }
        valid = true;
        console.log(
          `Complete creating username and password for user with email ${user.email}`,
        );
      }
      const token = authenticateMiddleware.generateAccessToken({
        username: userName,
      });
      ...
      return {
        ...
        success: valid,
        message: "Validating user complete",
        user: userRes,
      };
    } catch (error) {
