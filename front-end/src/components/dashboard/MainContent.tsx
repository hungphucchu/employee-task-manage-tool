...
const MainContent: React.FC = () => {
  ...
  return (
    <div className="main-content">
      ...
      <button onClick={handleOpenPopup} className="create-employee-button">
        + Create Employee
      </button>
      <input type="text" placeholder="Filter" className="filter-input" />
      <EmployeeTable
        employeeListUpdate={employeeListUpdate}
        onEmployeeListUpdate={() => setEmployeeListUpdate(!employeeListUpdate)}
      />
      <CreateEmployeePopup
        isOpen={isPopupOpen}
        onClose={handleClosePopup}
        onNewEmployeeAdded={() => setEmployeeListUpdate(!employeeListUpdate)}
      />
    </div>
  );
};
